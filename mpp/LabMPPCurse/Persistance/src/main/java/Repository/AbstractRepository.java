...
public abstract class AbstractRepository<ID, T extends HasId<ID>> implements IRepository<ID, T> {
    ...
    @Override
    public void update(ID id, T entity) throws ValidationException, RepositoryException {
        try {
    ...
    @Override
    public void save(T entity) throws RepositoryException, ValidationException {
        try {
            validator.validate(entity);
        }catch(ValidationException ex){
            System.err.println("Entitatea " +entity+ "nu este valida");

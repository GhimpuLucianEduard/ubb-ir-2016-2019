...
{	
	...
	{	
		...
		//private static IRepository<int,Client> _repository = new MockClientRepo();
		...
		{
			Client rez = null;
			_repository.FindAll().ToList().ForEach(c =>
			{
				if ((c.Nume.CompareTo(nume) == 0) && (c.Prenume.CompareTo(prenume) == 0))
				{
					rez = c;
				}
			});

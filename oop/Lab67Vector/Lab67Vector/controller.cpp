...
/*
	specificatii
*/
const vector<Car>& CarController::getAll()
{
	return repo.getAll();
}
...
vector<Car> CarController::producerFilt(string filt)
{
...
vector<Car> CarController::typeFilt(string filt)
{
	vector<Car> rez;
	for (const auto& car : repo.getAll())
	{
...
vector<Car> CarController::generalSort(bool(*lesser)(const Car&, const Car&))
{
	vector<Car> rez{ repo.getAll() };
...
vector<Car> CarController::sortById()
{
...
vector<Car> CarController::sortByProducatorModel()
{
	return generalSort([](const Car& c1, const Car& c2) {
